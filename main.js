...
`,Lr="color-history-store";let Rr=class extends at{constructor(){super(),this.history=[],this.activeIndex=-1,this.handleCommit=t=>{if(!(t instanceof Ft))return;const e=this.closest("color-picker");if(!e||!e.contains(t.target))return;const o=t.color.getHex();this.history.length>0&&this.history[0].getHex()===o||(this.history=[t.color,...this.history].slice(0,50),this.activeIndex=-1,this.saveToStorage(),this.saveLastColor(t.color))},this.history=this.loadFromStorage()}connectedCallback(){super.connectedCallback(),window.addEventListener(Ft.eventName,this.handleCommit)}saveLastColor(t){try{localStorage.setItem("last-active-color",t.getHex())}catch{}}disconnectedCallback(){super.disconnectedCallback(),window.removeEventListener(Ft.eventName,this.handleCommit)}selectSwatch(t,e){this.activeIndex=t,this.dispatchEvent(new jt(e)),this.saveLastColor(e)}clearHistory(){this.history=[],this.activeIndex=-1,this.saveToStorage()}saveToStorage(){try{const t=this.history.map(t=>({hex:t.getHex()}));localStorage.setItem(Lr,JSON.stringify(t))}catch{}}loadFromStorage(){try{const t=localStorage.getItem(Lr);if(!t)return[];const e=JSON.parse(t);return Array.isArray(e)?e.map(t=>new Pt({type:Ht.HEX,hex:t.hex})):[]}catch{return[]}}render(){return F`
    ...
    `}};Or.styles=[Gt],Or=t([ct("other-tools")],Or);let Hr=class extends at{constructor(){super(),this.color=new Pt({type:Ht.RGB255,r:71,g:85,b:105}),this.coordinates={x:0,y:0,width:1,height:1},this.interpolationLeft=new Pt({type:Ht.RGB255,r:255,g:0,b:0}),this.interpolationRight=new Pt({type:Ht.RGB255,r:255,g:255,b:255}),this.interpolationActive=qt.NONE,this.paletteActiveIndex=-1,this.loadLastColor(),this.addEventListener(jt.eventName,t=>{t instanceof jt&&this.setColor(t.color)}),this.addEventListener(Kt.eventName,t=>{t instanceof Kt&&this.setCoordinates(t.coordinates)}),this.addEventListener(Ut.eventName,t=>{t instanceof Ut&&this.setInterpolationActive(t.active)}),this.addEventListener(Sr.eventName,t=>{t instanceof Sr&&(this.paletteActiveIndex=t.index)})}loadLastColor(){try{const t=localStorage.getItem("last-active-color");t&&(this.color=new Pt({type:Ht.HEX,hex:t}))}catch{}}setColor(t){this.color=t,this.interpolationActive===qt.LEFT?this.interpolationLeft=t:this.interpolationActive===qt.RIGHT&&(this.interpolationRight=t)}setCoordinates(t){this.coordinates=t}setInterpolationActive(t){this.interpolationActive=t}updateChildren(){Array.prototype.forEach.call(this.children,t=>{t instanceof ne?t.color=this.color:t instanceof Yi?(t.color=this.color,t.coordinates=this.coordinates):t instanceof hr?t.coordinates=this.coordinates:t instanceof Xt?(t.leftColor=this.interpolationLeft,t.rightColor=this.interpolationRight,t.activeColor=this.interpolationActive):t instanceof ur?t.color=this.color:t instanceof _r&&(t.activeEditingColor=this.color)})}updated(){document.body.style.background="#"+this.color.getHex(),this.updateChildren()}render(){return F`<slot class="main-container"></slot>`}};Hr.styles=[Gt,Yt],t([ut()],Hr.prototype,"color",void 0),t([ut()],Hr.prototype,"coordinates",void 0),t([ut()],Hr.prototype,"interpolationLeft",void 0),t([ut()],Hr.prototype,"interpolationRight",void 0),t([ut()],Hr.prototype,"interpolationActive",void 0),t([ut()],Hr.prototype,"paletteActiveIndex",void 0),Hr=t([ct("color-picker")],Hr);

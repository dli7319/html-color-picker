...
`,Lr="color-history-store";let Rr=class extends at{constructor(){super(),this.history=[],this.activeIndex=-1,this.handleCommit=t=>{if(!(t instanceof Ft))return;const e=this.closest("color-picker");if(!e||!e.contains(t.target))return;const o=t.color.getHex();this.history.length>0&&this.history[0].getHex()===o||(this.history=[t.color,...this.history].slice(0,50),this.activeIndex=-1,this.saveToStorage())},this.history=this.loadFromStorage()}connectedCallback(){super.connectedCallback(),window.addEventListener(Ft.eventName,this.handleCommit)}disconnectedCallback(){super.disconnectedCallback(),window.removeEventListener(Ft.eventName,this.handleCommit)}selectSwatch(t,e){this.activeIndex=t,this.dispatchEvent(new jt(e))}clearHistory(){this.history=[],this.activeIndex=-1,this.saveToStorage()}saveToStorage(){try{const t=this.history.map(t=>({hex:t.getHex()}));localStorage.setItem(Lr,JSON.stringify(t))}catch{}}loadFromStorage(){try{const t=localStorage.getItem(Lr);if(!t)return[];const e=JSON.parse(t);return Array.isArray(e)?e.map(t=>new Pt({type:Ht.HEX,hex:t.hex})):[]}catch{return[]}}render(){return F`
